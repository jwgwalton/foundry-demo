...
This is an idealized target architecture rather than the structure implemented
by the current demo. The demo's local-only dependency design is documented in
[Local Dummy Dependencies](LOCAL_DEPENDENCIES.md).
...
live Foundry environment. Only deployment combines immutable images, specific
toolbox versions, and environment configuration.
